...
*                             
*  Output:  
*           1) Enrollment table at /etl/cdm_v41 
...
%macro etl_enrollment;
  ...
  %do yr = &startyr %to &endyr;
   ...
   proc sql; 
        select distinct memname into :denomtb from dictionary.tables 
        where lowcase(libname)="lib_den" and lowcase(memname) ? "&MBSFTBL" 
        and memname ? "&yr" and memtype='DATA';
    quit;

    data denom&yr(keep=BENE_ID BUYIN HMOIND);
      set lib_den.&denomtb(keep=&ENROLL_KEEP. %if &nametype eq L %then %do; rename=(&enroll_rn) %end;);
  ...
  %if %length(&PTDENOMTBL) > 0 %then %do; /* begin-if mbsf-D data is available to transform */
    ...
    %do yr = &pdstartyr %to &pdendyr;
	  proc sql;
	   ...
	   %end;
      from ref.colist
      where %if %eval(&yr >= 2016) %then %do; lowcase(strip(name_grp)) = "ptdenroll_k" %end; 
            %else %do; lowcase(strip(name_grp)) = "ptdenroll"; %end;;     
     quit;
  ...
  data pcordata.harvest;
	...
	REFRESH_ENROLLMENT_DATE = input("&sysdate9", date9.); 
  run;
