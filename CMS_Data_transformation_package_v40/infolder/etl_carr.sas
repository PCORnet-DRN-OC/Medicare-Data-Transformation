...
%macro assignvars_carr;
  ...
  FACILITY_LOCATION = ifc(length(PROVZIP)>5, substr(PROVZIP, 1,5), PROVZIP);
...
%macro ETL_CARR;
  ...
  proc sort data = craw_&yr out = carr_nodup_&yr nodupkey; by clm_id; run; 

  data carr_nodup_&yr;
    merge carr_nodup_&yr(in=a) carr_line&yr(in=b);        
    by clm_id;
	if a;
  run;

  proc sql;
    create table carr_raw_&yr as
	  select distinct * from carr_nodup_&yr(drop=clm_id);
  quit;
  ...
  data pcordata.carr_px_rev&yr;
    set carr_raw_&yr(keep=PATID ENCOUNTERID ENC_TYPE ADMIT_DATE PROVIDERID THRU_DT HCPCS_CD EXPNSDT1);
  run;
