...
*           claims data to Pcornet CDM 5.1 annual tables and then create final views/datasets 
...
*               Carrier claim table 
*               Carrier line table 
*               Snf claim table (if available)
*               Snf revenue table (if available) 
*               Part D drug event table (if available)
*               Mbsf_abcd/Mbsf_d table name (if available)
*               MedPAR table ( if available, set either MedPAR file or Inpatient and Snf as Input files) 
*
*           2) SAS programs:
...
*                             etl_pde.sas
*                             build_provider.sas
*                             create_CDM_v51_tablesviews.sas  
*                             build_xwlks.sas
*                             build_harvest.sas
*
*  Output:
*           1) Annual PCORnet CDM v5.1 SAS data datasets at /etl/cdm_v51 
*           2) Final CDM SAS data datasets or views stored at /etl/cdm_v51 
*           3) Empty CDM SAS data datasets where no Medicare data are populated at /etl/cdm_v51  
*           4) SAS log files of each programs stored in /etl/outfolder
*           5) SAS output file of each programs per year in PDF format stored in /etl/outfolder
*
*  Requirements:  
...
*                     Notes: The MedPAR file is an alternate source of information about inpatient hospital and SNF care, users can provide 
*                            either MedPAR table name or Inpatient Claims and SNF Claims table names, 
*                            but not both, if all tables are available.
*               4) nametype    -- Flag specifies whether long or short column names used in Medicare Source tables 
*               5) mkxwlk_hrvs -- Flag indicates whether harvest table, and provider and facility cross walk files need to 
*                                 be created or refreshed             
*               6)     startyr -- First year of MBSF, Part A, B source data to transform 
*               7)       endyr -- Last year of MBSF, Part A, B source data to transform  
*               8)   pdstartyr -- First year of Part D source data to transform 
*               9)     pdendyr -- Last year of Part D source data to transform 
*               10)     dsview -- Flag indicates whether or not you want to combine the Annual tables to single views 
*               11) User provides information on network-id, network-name, datamart-id, datamart-name, datamart-platform 
*               
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */;
options mprint nocenter validvarname=upcase;
%macro PCORNET_CDM_ETL_MASTER_FILE;

/* Enter root directory to sub-directory for ETL workplans */
%let epath = /xxxx/yyyy/CMS_Data_Transformation_Package_v40/;

/* Enter main directory where the source data tables are stored */
%let sdpath = /aaaa/bbbb/;
...
%let SDSNF    = ;  /*Edit sub-directory for Inpatient claim data   */;
...
%let SDMP     = ;  /*Edit sub-directory for Medpar data */;
...
%let MBSFTBL    = mbsf_abc;   /*Edit the MBSF_AB/MBSF_ABCD table name     */;        
%let IPTBL      = inptclmsk; /*Edit the Inpatient claim base table name. Leave blank if MedPAR table is available*/;
%let IPREVTBL   = inptrevk;  /*Edit the Inpatient revenue table name. Leave blank if MedPAR table is available*/; 
%let SNFTBL     = snfclmsk;  /*Edit the SNF claim base table name. Leave blank if MedPAR table is available*/;
%let SNFREVTBL  = snfrevk;        /*Edit the SNF revenue table name. Leave blank if MedPAR table is available*/; 
%let MPTBL      = ;  /*Edit the Medpar table name. Leave blank if Inpatient tables are available*/;
%let OPTBL      = otptclmsk;/*Edit the Outpatient claim base table name */;
%let OPREVTBL   = otptrevk; /*Edit the Outpatient revenue name          */; 
%let CARTBL     = bcarclmsk;/*Edit the Carrier claim base table name    */;
%let CARLNTBL   = bcarlinek;/*Edit the Carrier line table name          */;
%let PDETBL     = pdesaf;  /*Edit the Part D drug event table name     */;
%let PTDENOMTBL = mbsf_d;  /*Edit the MBSF_D/MBSF_ABCD table name      */;

/*- Set nametype to S if CMS Medicare source tables with Short Column Names  -*/;
...
* Set dsview to N if you want to generate tables only 
********************************************************************************/

%let MKXWLK_HRVS = Y;     /* Default is Y. Set to N if harvest table, provider table and facility crosswalk table have already been created. */;
%let STARTYR     = 2016;  /* Set first year of MBSF, Part A, B source data to transform. */;
%let ENDYR       = 2017;  /* Set last year of MBSF, Part A, B source data to transform. */;
%let PDSTARTYR   = 2016;  /* Set first year of Part D source data to transform. */; 
%let PDENDYR     = 2017;  /* Set last year of Part D source data to transform. */;   
%let DSVIEW      = N;     /* Default is N. Set to Y if you only want to combine the Annual tables to single views, set to B to create both permanent datasets and views. */;

/* Specify network-id, network-name, datamart-id, datamart-name, datamart-platform */;
%let NETID  = CMS;
%let NETNM  = CMS;
%let DMID   = FIVEPCT;
%let DMNM   = CMS 5% SAMPLE;
%let DMPLFM = 05;
...
%let ADDRTBL=lds_address_history /*specify the address table name       */;
...
%let PROVTBL=provider;           /*specify the dispensing table name  */; 
...
libname lib_snf "&sdpath.&sdsnf" access=readonly;
...
libname lib_mp "&sdpath.&sdmp" access=readonly;
libname pcordata "&epath.cdm_v51/";
libname ref "&epath.infolder/";

/********************************************************************************
* Import transport file for the reference data sets
*********************************************************************************/;
filename tranfile "&epath.infolder/reference_file.cpt";
proc cimport infile=tranfile library=ref; run;
...
%if &mkxwlk_hrvs eq y %then %do ; 
  ...
  %inc "&epath.infolder/build_provider.sas" /nosource2 ;
%end;
...
%if %sysevalf(&syscc gt 4) %then %do ;
   %put .................................................................................;
   %put etl_denom macro is aborting;                 
   %put Check program log &epath.infolder/etl_denom.log for details;                
   %put .................................................................................;      
   %abort cancel ;      
 %end;
...
%if %sysevalf(&syscc gt 4) %then %do ;
 ...
 proc sort data = pcordata.facility(keep=SOURCE_ID FACILITYID) 
  ...
  proc sort data = pcordata.provider 
       out=provider(keep=PROVIDER_NPI PROVIDERID) nodupkey;
       by PROVIDER_NPI;
  run;

  data provider;
    length AT_NPI $10.;
    set pcordata.provider(keep=PROVIDER_NPI PROVIDERID);
    AT_NPI = put(PROVIDER_NPI, 10.);
  run;

  proc sort data = provider nodupkey; by AT_NPI; run;

  data demographic; set pcordata.demographic(keep=PATID); run;
  ...
  %do yr = &startyr %to &endyr;
	...
	%else %if %length(&MPTBL) > 0 %then %do; /* begin-if MedPAR data is available to transform */
      %include "&epath.infolder/etl_medpar.sas";
      %if %sysevalf(&syscc gt 4) %then %do ;
        %put .................................................................................;
        %put etl_medpar macro is aborting;                 
        %put Check program log &epath.infolder/etl_etl_medpar.log for details;                                           
        %put .................................................................................;      
        %abort cancel ;      
      %end;
    %end; /* end-if etl_medpar data is available to transform */
    ...
    %if %length(&SNFTBL) > 0 and (%length(&IPTBL) > 0 or %length(&MPTBL) = 0) %then %do; /* begin-if snf data is available to transform */
      %include "&epath.infolder/etl_snf.sas";
      %if %sysevalf(&syscc gt 4) %then %do ;
        %put .................................................................................; 
        %put etl_snf macro is aborting;                 
        %put Check program log &epath.infolder/snf_carr.log for details;                                           
        %put .................................................................................;      
        %abort cancel ;      
      %end; 
    %end; /* end-if snf data is available to transform */
   %end;/*end of yr loop*/
   ...
   %inc "&epath.infolder/create_CDM_v51_tablesviews.sas";
