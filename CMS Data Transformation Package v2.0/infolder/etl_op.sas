...
%macro ETL_OP;
  ...
  data op_raw_&yr;
    set lib_op.&OPTBL&yr(keep=&OP_KEEP %if &nametype eq L %then %do; rename=(&OP_RN) %end;);
  run;
  ...
  proc sql;
    create table op_rev_&yr as   
      select distinct a.ENCOUNTERID,
			  ...
			  b.REV_CNTR,
			  b.REV_DT
	  from op_raw_&yr a, lib_op.&OPREVTBL&yr(keep=&OPREV_KEEP %if &nametype eq L %then %do; rename=(&OPREV_RN) %end;) b
