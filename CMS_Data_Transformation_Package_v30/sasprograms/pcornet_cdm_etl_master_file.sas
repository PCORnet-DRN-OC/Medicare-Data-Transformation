...
*           claims data to Pcornet CDM 4.1 annual tables and then create final views/datasets 
...
*                             etl_pde.sas
*                             build_provider.sas
*                             create_CDM_v41_tablesviews.sas  
*                             build_xwlks.sas
*                             build_harvest.sas
*
*  Output:
*           1) Annual PCORnet CDM v4.1 SAS data datasets at /etl/cdm_v41 
*           2) Final CDM SAS data datasets or views stored at /etl/cdm_v41 
*           3) Empty CDM SAS data datasets where no Medicare data are populated at /etl/cdm_v41  
...
*               
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */;
options mprint nocenter fullstimer validvarname=upcase;
...
%let IPTBL      = ;        /*Edit the Inpatient claim base table name. Leave blank if MedPAR table is available*/;
%let IPREVTBL   = ;        /*Edit the Inpatient revenue table name. Leave blank if MedPAR table is available*/; 
%let SNFTBL     = ;        /*Edit the SNF claim base table name. Leave blank if MedPAR table is available*/;
%let SNFREVTBL  = ;        /*Edit the SNF revenue table name. Leave blank if MedPAR table is available*/; 
%let MPTBL      = medpar;  /*Edit the Medpar table name. Leave blank if Inpatient tables are available*/;
%let OPTBL      = otptclms;/*Edit the Outpatient claim base table name */;
%let OPREVTBL   = otptrev; /*Edit the Outpatient revenue name          */; 
%let CARTBL     = bcarclms;/*Edit the Carrier claim base table name    */;
%let CARLNTBL   = bcarline;/*Edit the Carrier line table name          */;
...
libname pcordata "&epath.cdm_v41/";
...
%if %sysevalf(&syscc gt 4) %then %do ;
 ...
 proc sort data = pcordata.facility(keep=SOURCE_ID FACILITYID) 
  ...
  %inc "&epath.infolder/create_CDM_v41_tablesviews.sas";
